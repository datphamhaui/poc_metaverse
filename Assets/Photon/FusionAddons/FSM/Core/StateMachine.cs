...
namespace Fusion.FSM
{
	public partial class StateMachine<TState> : IStateMachine where TState : class, IState
	{
		...
		private void LogStateChange()
		{
			...
			if (HasStarted == true)
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#F04C4C>State Machine <b>{Name}</b>: Change State to <b>{ActiveState.Name}</b></color> - Previous: {PreviousState.Name}, Tick: {Runner.Simulation.Tick.Raw}", _controller);
			}
			else
			{
				Debug.Log($"{_controller.gameObject.name} - <color=#F04C4C>State Machine <b>{Name}</b>: Change State to <b>{ActiveState.Name}</b></color> - Tick: {Runner.Simulation.Tick.Raw}", _controller);
			}
		}
